      ...
      <Form v-if="allowRegister" ref="formInline" :model="formInline" :rules="ruleInline" inline>
        ...
        <FormItem prop="user">
          <Input type="text" v-model="formInline.user" :placeholder="key">
            <Select v-model="country" slot="prepend"
                    style="width: 65px;border-bottom: 1px solid #27313e;">
              <Option value="China" label="+86"><span>+86</span><span
                  style="margin-left:10px;color:#ccc">China</span></Option>
              <Option value="Singapore" label="+65"><span>+65</span><span
                  style="margin-left:10px;color:#ccc">Singapore</span></Option>
              <Option value="South Korea" label="+82"><span>+82</span><span
                  style="margin-left:10px;color:#ccc">South Korea</span></Option>
              <Option value="Nigeria" label="+234"><span>+234</span><span
                  style="margin-left:10px;color:#ccc">Nigeria</span></Option>
              <Option value="日本" label="+81"><span>+81</span><span
                  style="margin-left:10px;color:#ccc">日本</span></Option>
              <Option value="泰国" label="+66"><span>+66</span><span
                  style="margin-left:10px;color:#ccc">泰国</span></Option>
              <Option value="俄罗斯" label="+7"><span>+7</span><span
                  style="margin-left:10px;color:#ccc">俄罗斯</span></Option>
              <Option value="英国" label="+44"><span>+44</span><span
                  style="margin-left:10px;color:#ccc">英国</span></Option>
              <Option value="越南" label="+84"><span>+84</span><span
                  style="margin-left:10px;color:#ccc">越南</span></Option>
              <Option value="印度" label="+91"><span>+91</span><span
                  style="margin-left:10px;color:#ccc">印度</span></Option>
              <Option value="意大利" label="+39"><span>+39</span><span
                  style="margin-left:10px;color:#ccc">意大利</span></Option>
              <Option value="香港" label="+852"><span>+852</span><span
                  style="margin-left:10px;color:#ccc">香港</span></Option>
              <Option value="马来西亚" label="+60"><span>+60</span><span
                  style="margin-left:10px;color:#ccc">马来西亚</span></Option>
              <Option value="台湾省" label="+886"><span>+886</span><span
                  style="margin-left:10px;color:#ccc">台湾省</span></Option>
              <Option value="土耳其" label="+90"><span>+90</span><span
                  style="margin-left:10px;color:#ccc">土耳其</span></Option>
            </Select>
          </Input>
        </FormItem>
        ...
        <FormItem prop="code" v-show="showCode">
          ...
          <input id="sendCode" @click="sendCode();" type="Button" shape="circle"
                 :value="sendcodeValue" :disabled='codedisabled'>
          </input>
        </FormItem>
        ...
        <div class="check-agree" style="">
          ...
          <a v-if="lang=='简体中文'" href="/helpdetail?cate=1&id=5&cateTitle=常见问题" target="_blank"
             style="">《{{ $t('uc.regist.userprotocol') }}》</a>
          <a v-if="lang=='English'" href="/helpdetail?cate=1&id=35&cateTitle=Privacy Policy"
             target="_blank" style="">《{{ $t('uc.regist.userprotocol') }}》</a>
        </div>
        <FormItem>
...
export default {
  data() {
    const validateUser = (rule, value, callback) => {
      if (this.changeActive == 0) {
        ...
        } else if (!this.isValidPhonenumber(this.formInline.user)) {
  ...
  methods: {
    ...
    sendCode() {
      ...
      if (mobilePhone == "" && !this.isValidPhonenumber(mobilePhone)) {
    ...
    success() {
      ...
      this.isValidPhonenumber(params["phone"]) && this.$http.post(this.host + "/uc/mobile/code", params).then(
          response => {
            var resp = response.body;
            resp.code == 0 && this.$Notice.success({
              title: this.$t("common.tip"),
              desc: resp.message
            });
            resp.code == 0 && this.settime();
            resp.code != 0 && this.$Notice.error({
              title: this.$t("common.tip"),
              desc: resp.message
            });
          });
      !reg.test(params["phone"]) && this.$Notice.error({
        title: this.$t("common.tip"),
